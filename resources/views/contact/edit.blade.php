<x-layout title="create contact">
    <form action="{{ route('contact.update', $contact->id) }}" class="form" method="post" enctype="multipart/form-data">
        @csrf
        @method('put')
        <input type="file" accept="image/*" id="avatar" name="avatar" hidden>
        @if ($contact->avatar)
            <div class="avatar-image " id="image_avatar" style="position: relative">
                <img src="{{ Storage::disk(\App\Models\Contact::DISK)->url($contact->avatar) }}" id="avatar_image"
                    alt=""
